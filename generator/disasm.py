# Source -  https://github.com/mengstr/pdp8emu/blob/master/disasm.c
...
operations = [
	...
	"CLDI       DK8-EA: Disable Clock InterruptOctal error)",  # op 6132
	...
	"CDF 0 CIF 0KM8-E: Change to Data and Instruction Field 0",  # op 6203
	...
	"CDF 1 CIF 1KM8-E: Change to Data and Instruction Field 1",  # op 6213
	...
	"CDF 2 CIF 2KM8-E: Change to Data and Instruction Field 2",  # op 6223
	...
	"CDF 3 CIF 3KM8-E: Change to Data and Instruction Field 3",  # op 6233
	...
	"CDF 4 CIF 4KM8-E: Change to Data and Instruction Field 4",  # op 6243
	...
	"CDF 5 CIF 5KM8-E: Change to Data and Instruction Field 5",  # op 6253
	...
	"CDF 6 CIF 6KM8-E: Change to Data and Instruction Field 6",  # op 6263
	...
	"CDF 7 CIF 7KM8-E: Change to Data and Instruction Field 7",  # op 6273
	...
	"CLA IAC BSWSet AC to 0100",  # op 7203
	"GLK        Move L into AC",  # op 7204
	"CLA IAC RALSet AC to 0001, Rotate AC & L left",  # op 7205
	"CLA RTL    Move L into AC bit 10",  # op 7206
	"CLA IAC RTLSet AC to 0001, Rotate AC & L left twice",  # op 7207
	"CLA RAR    Move L into AC bit 0",  # op 7210
	"CLA IAC RARSet AC to 0001, Rotate AC & L right",  # op 7211
	"CLA RTR    Move L into AC bit 1",  # op 7212
	"CLA IAC RTRSet AC to 0001, Rotate AC & L right twice",  # op 7213
	"           ",  # op 7214
	"           ",  # op 7215
	"           ",  # op 7216
	"           ",  # op 7217
	"CML CLA    Complement L, Clear AC",  # op 7220
	"CML CLA IACComplement L, Set AC to 0001",  # op 7221
	"CML CLA BSWComplement L, Clear AC",  # op 7222
	"CML CLA IAC BSWComplement L, set AC to 0100",  # op 7223
	"CML GLK    Complement L, Move L into AC",  # op 7224
	"CML CLA IAC RALComplement L, Set AC to 0001, Rotate AC & L left",  # op 7225
	"CML CLA RTLComplement L, Move L into AC bit 10",  # op 7226
	"CML CLA IAC RTLComplement L, Set AC to 0001, Rotate AC & L left twice",  # op 7227
	"CML CLA RARComplement L, Move L into AC bit 0",  # op 7230
	"CML CLA IAC RARComplement L, Set AC to 0001, Rotate AC & L right",  # op 7231
	"CML CLA RTRComplement L, Move L into AC bit 1",  # op 7232
	"CML CLA IAC RTRComplement L, Set AC to 0001, Rotate AC & L right twice",  # op 7233
	...
	"CLA CMA IACSet AC to 0, set L",  # op 7241
	"CLA CMA BSWSet AC to 7777, Byte swap AC",  # op 7242
	"CLA CMA IAC BSWClear AC",  # op 7243
	"CLA CMA RALSet AC to 7777, rotate AC & L left",  # op 7244
	"CLA CMA IAC RALMove L into AC",  # op 7245
	"CLA CMA RTLSet AC to 7777, rotate AC & L left twice",  # op 7246
	"CLA CMA IAC RTLMove L into AC bit 10",  # op 7247
	"CLA CMA RARSet AC to 7777, rotate AC & L right",  # op 7250
	"CLA CMA IAC RARMove L into AC bit 0",  # op 7251
	"CLA CMA RTRSet AC to 7777, rotate AC & L right twice",  # op 7252
	"CLA CMA IAC RTRMove L into AC bit 1",  # op 7253
	"           ",  # op 7254
	"           ",  # op 7255
	"           ",  # op 7256
	"           ",  # op 7257
	"CML STA    Complement L, Set AC to 7777",  # op 7260
	"CML CLA CMA IACComplement L, Set AC to 0, set L",  # op 7261
	"CML CLA CMA BSWComplement L, Set AC to 7777, Byte swap AC",  # op 7262
	"CML CLA CMA IAC BSWComplement L, Clear AC",  # op 7263
	"CML CLA CMA RALComplement L, Set AC to 7777, rotate AC & L left",  # op 7264
	"CML CLA CMA IAC RALComplement L, Move L into AC",  # op 7265
	"CML CLA CMA RTLComplement L, Set AC to 7777, rotate AC & L left twice",  # op 7266
	"CML CLA CMA IAC RTLComplement L, Move L into AC bit 10",  # op 7267
	"CML CLA CMA RARComplement L, Set AC to 7777, rotate AC & L right",  # op 7270
	"CML CLA CMA IAC RARComplement L, Move L into AC bit 0",  # op 7271
	"CML CLA CMA RTRComplement L, Set AC to 7777, rotate AC & L right twice",  # op 7272
	"CML CLA CMA IAC RTRComplement L, Move L into AC bit 1",  # op 7273
	...
	"CLL CLA IACClear L, Set AC to 0001",  # op 7301
	"CLL CLA BSWClear L, Clear AC",  # op 7302
	"CLL CLA IAC BSWClear L, set AC to 0100",  # op 7303
	"CLL GLK    Clear L, Move L into AC",  # op 7304
	"CLL CLA IAC RALClear L, Set AC to 0001, Rotate AC & L left",  # op 7305
	"CLL CLA RTLClear L, Move L into AC bit 10",  # op 7306
	"CLL CLA IAC RTLClear L, Set AC to 0001, Rotate AC & L left twice",  # op 7307
	"CLL CLA RARClear L, Move L into AC bit 0",  # op 7310
	"CLL CLA IAC RARClear L, Set AC to 0001, Rotate AC & L right",  # op 7311
	"CLL CLA RTRClear L, Move L into AC bit 1",  # op 7312
	"CLL CLA IAC RTRClear L, Set AC to 0001, Rotate AC & L right twice",  # op 7313
	"CLL        Clear L",  # op 7314
	"CLL        Clear L",  # op 7315
	"CLL        Clear L",  # op 7316
	"CLL        Clear L",  # op 7317
	"STL CLA    Set L, Clear AC",  # op 7320
	"STL CLA IACSet L, Set AC to 0001",  # op 7321
	"STL CLA BSWSet L, Clear AC",  # op 7322
	"STL CLA IAC BSWSet L, set AC to 0100",  # op 7323
	"STL GLK    Set L, Move L into AC",  # op 7324
	"STL CLA IAC RALSet L, Set AC to 0001, Rotate AC & L left",  # op 7325
	"STL CLA RTLSet L, Move L into AC bit 10",  # op 7326
	"STL CLA IAC RTLSet L, Set AC to 0001, Rotate AC & L left twice",  # op 7327
	"STL CLA RARSet L, Move L into AC bit 0",  # op 7330
	"STL CLA IAC RARSet L, Set AC to 0001, Rotate AC & L right",  # op 7331
	"STL CLA RTRSet L, Move L into AC bit 1",  # op 7332
	"STL CLA IAC RTRSet L, Set AC to 0001, Rotate AC & L right twice",  # op 7333
	"STL        Set L",  # op 7334
	"STL        Set L",  # op 7335
	"STL        Set L",  # op 7336
	"STL        Set L",  # op 7337
	"CLL STA    Clear L, Set AC to 7777",  # op 7340
	"CLL CLA CMA IACClear L, Set AC to 0, set L",  # op 7341
	"CLL CLA CMA BSWClear L, Set AC to 7777, Byte swap AC",  # op 7342
	"CLL CLA CMA IAC BSWClear L, Clear AC",  # op 7343
	"CLL CLA CMA RALClear L, Set AC to 7777, rotate AC & L left",  # op 7344
	"CLL CLA CMA IAC RALClear L, Move L into AC",  # op 7345
	"CLL CLA CMA RTLClear L, Set AC to 7777, rotate AC & L left twice",  # op 7346
	"CLL CLA CMA IAC RTLClear L, Move L into AC bit 10",  # op 7347
	"CLL CLA CMA RARClear L, Set AC to 7777, rotate AC & L right",  # op 7350
	"CLL CLA CMA IAC RARClear L, Move L into AC bit 0",  # op 7351
	"CLL CLA CMA RTRClear L, Set AC to 7777, rotate AC & L right twice",  # op 7352
	"CLL CLA CMA IAC RTRClear L, Move L into AC bit 1",  # op 7353
	"CLL        Clear L",  # op 7354
	"CLL        Clear L",  # op 7355
	"CLL        Clear L",  # op 7356
	"CLL        Clear L",  # op 7357
	"STL STA    Set L, Set AC to 7777",  # op 7360
	"STL CLA CMA IACSet L, Set AC to 0, set L",  # op 7361
	"STL CLA CMA BSWSet L, Set AC to 7777, Byte swap AC",  # op 7362
	"STL CLA CMA IAC BSWSet L, Clear AC",  # op 7363
	"STL CLA CMA RALSet L, Set AC to 7777, rotate AC & L left",  # op 7364
	"STL CLA CMA IAC RALSet L, Move L into AC",  # op 7365
	"STL CLA CMA RTLSet L, Set AC to 7777, rotate AC & L left twice",  # op 7366
	"STL CLA CMA IAC RTLSet L, Move L into AC bit 10",  # op 7367
	"STL CLA CMA RARSet L, Set AC to 7777, rotate AC & L right",  # op 7370
	"STL CLA CMA IAC RARSet L, Move L into AC bit 0",  # op 7371
	"STL CLA CMA RTRSet L, Set AC to 7777, rotate AC & L right twice",  # op 7372
	"STL CLA CMA IAC RTRSet L, Move L into AC bit 1",  # op 7373
	"STL        Set L",  # op 7374
	"STL        Set L",  # op 7375
	"STL        Set L",  # op 7376
	"STL        Set L",  # op 7377
	...
	"HLT        Halt",  # op 7402
	"SCL        Step counter loaddepends on mode)",  # op 7403
	...
	"SKP OSR HLTSkip unconditionally, OR Switch Register with AC, Halt",  # op 7416
	"LSR        Logical shift right",  # op 7417
	"SNL        Skip on L = 1",  # op 7420
	"MQL        Load MQ from AC then clear AC",  # op 7421
	"SNL HLT    Skip on L = 1, Halt",  # op 7422
	"MQL SCL    Load MQ from AC then clear AC, Step counter load",  # op 7423
	"SNL OSR    Skip on L = 1, OR Switch Register with AC",  # op 7424
	"MQL MUY    Load MQ from AC then clear AC, Multiply",  # op 7425
	"SNL OSR HLTSkip on L = 1, OR Switch Register with AC, Halt",  # op 7426
	...
	"SZL OSR HLTSkip on L = 0, OR Switch Register with AC, Halt",  # op 7436
	...
	"DAD        Double precision addMode B)",  # op 7443
	"SZA OSR    Skip on AC = 0, OR Switch Register with AC",  # op 7444
	"DST        Double precision storeMode B)",  # op 7445
	"SZA OSR HLTSkip on AC = 0, OR Switch Register with AC, Halt",  # op 7446
	"SWBA       Switch mode from B to A",  # op 7447
	"SNA        Skip on AC != 0",  # op 7450
	"DPSZ       Double precision skip if zeroMode B)",  # op 7451
	...
	"SNA OSR HLTSkip on AC != 0, OR Switch Register with AC, Halt",  # op 7456
	"SAM        Subtract AC from MQ",  # op 7457
	"SZA SNL    Skip on AC = 0 or L = 1",  # op 7460
	"UNKNOWN!   **Unknown**",  # op 7461
	"SZA SNL HLTSkip on AC = 0 or L = 1, Halt",  # op 7462
	"UNKNOWN!   **Unknown**",  # op 7463
	"SZA SNL OSRSkip on AC = 0 or L = 1, OR Switch Register with AC",  # op 7464
	"UNKNOWN!   **Unknown**",  # op 7465
	"SZA SNL OSR HLTSkip on AC = 0 or L = 1, OR Switch Register with AC, Halt",  # op 7466
	"UNKNOWN!   **Unknown**",  # op 7467
	"SNA SZL    Skip on AC != 0 and L = 0",  # op 7470
	"UNKNOWN!   ILLEGAL!",  # op 7471
	"SNA SZL HLTSkip on AC != 0 and L = 0, Halt",  # op 7472
	"UNKNOWN!   **Unknown**",  # op 7473
	"SNA SZL OSRSkip on AC != 0 and L = 0, OR Switch Register with AC",  # op 7474
	"UNKNOWN!   **Unknown**",  # op 7475
	"SNA SZL OSR HLTSkip on AC != 0 and L = 0, OR Switch Register with AC, Halt",  # op 7476
	...
	"SMA OSR HLTSkip on AC < 0, OR Switch Register with AC, Halt",  # op 7506
	...
	"SPA OSR HLTSkip on AC >= 0, OR Switch Register with AC, Halt",  # op 7516
	"MQA LSR    OR MQ with AC, Logical shift right",  # op 7517
	"SMA SNL    Skip on AC < 0 or L = 1",  # op 7520
	"SWP        Swap MQ and AC",  # op 7521
	"SMA SNL HLTSkip on AC < 0 or L = 1, Halt",  # op 7522
	"SWP SCL    Swap AC and MQ, Step counter load",  # op 7523
	"SMA SNL OSRSkip on AC < 0 or L = 1, OR Switch Register with AC",  # op 7524
	"SWP MUY    Swap AC and MQ, Multiply",  # op 7525
	"SMA SNL OSR HLTSkip on AC < 0 or L = 1, OR Switch Register with AC, Halt",  # op 7526
	"SWP DVI    Swap AC and MQ, Divide",  # op 7527
	"SPA SZL    Skip on AC >= 0 and L = 0",  # op 7530
	"UNKNOWN!   ILLEGAL!",  # op 7531
	"SPA SZL HLTSkip on AC >= 0 and L = 0, Halt",  # op 7532
	"SWP SHL    Swap AC and MQ, Shift left",  # op 7533
	"SPA SZL OSRSkip on AC >= 0 and L = 0, OR Switch Register with AC",  # op 7534
	"SWP ASR    Swap AC and MQ, Arithmetic shift right",  # op 7535
	"SPA SZL OSR HLTSkip on AC >= 0 and L = 0, OR Switch Register with AC, Halt",  # op 7536
	"SWP LSR    Swap AC and MQ, Logical shift right",  # op 7537
	"SMA SZA    Skip on AC <= 0",  # op 7540
	"UNKNOWN!   **Unknown**",  # op 7541
	"SMA SZA HLTSkip on AC <= 0, Halt",  # op 7542
	"UNKNOWN!   **Unknown**",  # op 7543
	"SMA SZA OSRSkip on AC <= 0, OR Switch Register with AC",  # op 7544
	"UNKNOWN!   **Unknown**",  # op 7545
	"SMA SZA OSR HLTSkip on AC <= 0, OR Switch Register with AC, Halt",  # op 7546
	"UNKNOWN!   **Unknown**",  # op 7547
	"SPA SNA    Skip on AC > 0",  # op 7550
	"UNKNOWN!   ILLEGAL!",  # op 7551
	"SPA SNA HLTSkip on AC > 0, Halt",  # op 7552
	"UNKNOWN!   **Unknown**",  # op 7553
	"SPA SNA OSRSkip on AC > 0, OR Switch Register with AC",  # op 7554
	"UNKNOWN!   **Unknown**",  # op 7555
	"SPA SNA OSR HLTSkip on AC > 0, OR Switch Register with AC, Halt",  # op 7556
	"UNKNOWN!   **Unknown**",  # op 7557
	"SMA SZA SNLSkip on AC <= 0 or L = 1",  # op 7560
	"UNKNOWN!   **Unknown**",  # op 7561
	"SMA SZA SNL HLTSkip on AC <= 0 or L = 1, Halt",  # op 7562
	"UNKNOWN!   **Unknown**",  # op 7563
	"SMA SZA SNL OSRSkip on AC <= 0 or L = 1, OR Switch Register with AC",  # op 7564
	"UNKNOWN!   **Unknown**",  # op 7565
	"SMA SZA SNL OSR HLTSkip on AC <= 0 or L = 1, OR Switch Register with AC, Halt",  # op 7566
	"UNKNOWN!   **Unknown**",  # op 7567
	"SPA SNA SZLSkip on AC > 0 and L=0",  # op 7570
	"UNKNOWN!   ILLEGAL!",  # op 7571
	"SPA SNA SZL HLTSkip on AC > 0 and L=0, Halt",  # op 7572
	"DPIC       Double precision incrementMode B)",  # op 7573
	"SPA SNA SZL OSRSkip on AC > 0 and L=0, OR Switch Register with AC",  # op 7574
	"DCM        Double precision complementMode B)",  # op 7575
	"SPA SNA SZL OSR HLTSkip on AC > 0 and L=0, OR Switch Register with AC, Halt",  # op 7576
	...
	"SKP CLA HLTSkip unconditionally, Clear AC, Halt",  # op 7612
	...
	"SKP LAS HLTSkip unconditionally, Load AC from the Switch Register, Halt",  # op 7616
	"CLA LSR    Clear AC, Logical shift right",  # op 7617
	"SNL CLA    Skip on L = 1, Clear AC",  # op 7620
	"CAM        Clear AC and MQ",  # op 7621
	"SNL CLA HLTSkip on L = 1, Clear AC, Halt",  # op 7622
	"CAM SCL   Clear AC and MQ, Step counter load",  # op 7623
	"SNL LAS    Skip on L = 1, Load AC from the Switch Register",  # op 7624
	"CAM MUY   Clear AC and MQ, Multiply",  # op 7625
	"SNL LAS HLTSkip on L = 1, Load AC from the Switch Register, Halt",  # op 7626
	"CAM DVI   Clear AC and MQ, Divide",  # op 7627
	"SZL CLA    Skip on L = 0, Clear AC",  # op 7630
	"UNKNOWN!   ILLEGAL!",  # op 7631
	"SZL CLA HLTSkip on L = 0, Clear AC, Halt",  # op 7632
	...
	"SZL LAS HLTSkip on L = 0, Load AC from the Switch Register, Halt",  # op 7636
	...
	"SZA CLA HLTSkip on AC = 0, Clear AC, Halt",  # op 7642
	"CLA DAD    Clear AC, Double precision add    Mode B)",  # op 7643
	"SZA LAS    Skip on AC = 0, Load AC from the Switch Register",  # op 7644
	"CLA DST    Clear AC, Double precision storeMode B)",  # op 7645
	"SZA LAS HLTSkip on AC = 0, Load AC from the Switch Register, Halt",  # op 7646
	"CLA SWBA   Clear AC, Switch mode from B to A",  # op 7647
	"SNA CLA    Skip on AC != 0, Clear AC",  # op 7650
	"UNKNOWN!   ILLEGAL!",  # op 7651
	"SNA CLA HLTSkip on AC != 0, Clear AC, Halt",  # op 7652
	"UNKNOWN!   **Unknown**",  # op 7653
	"SNA LAS    Skip on AC != 0, Load AC from the Switch Register",  # op 7654
	"UNKNOWN!   **Unknown**",  # op 7655
	"SNA LAS HLTSkip on AC != 0, Load AC from the Switch Register, Halt",  # op 7656
	"CLA SAM    Clear AC, Subtract AC from MQ",  # op 7657
	"SZA SNL CLASkip on AC = 0 or L = 1, Clear AC",  # op 7660
	"UNKNOWN!   **Unknown**",  # op 7661
	"SZA SNL CLA HLTSkip on AC = 0 or L = 1, Clear AC, Halt",  # op 7662
	"UNKNOWN!   **Unknown**",  # op 7663
	"SZA SNL LASSkip on AC = 0 or L = 1, Load AC from the Switch Register",  # op 7664
	"UNKNOWN!   **Unknown**",  # op 7665
	"SZA SNL LAS HLTSkip on AC = 0 or L = 1, Load AC from the Switch Register, Halt",  # op 7666
	"UNKNOWN!   **Unknown**",  # op 7667
	"SNA SZL CLASkip on AC != 0 and L = 0, Clear AC",  # op 7670
	"UNKNOWN!   ILLEGAL!",  # op 7671
	"SNA SZL CLA HLTSkip on AC != 0 and L = 0, Clear AC, Halt",  # op 7672
	"UNKNOWN!   **Unknown**",  # op 7673
	"SNA SZL LASSkip on AC != 0 and L = 0, Load AC from the Switch Register",  # op 7674
	"UNKNOWN!   **Unknown**",  # op 7675
	"SNA SZL LAS HLTSkip on AC != 0 and L = 0, Load AC from the Switch Register, Halt",  # op 7676
	"UNKNOWN!   **Unknown**",  # op 7677
	"SMA CLA    Skip on AC < 0, Clear AC",  # op 7700
	"ACL        Load MQ into AC",  # op 7701
	"SMA CLA HLTSkip on AC < 0, Clear AC, Halt",  # op 7702
	...
	"SMA LAS HLTSkip on AC < 0, Load AC from the Switch Register, Halt",  # op 7706
	"ACL DVI    Load MQ into AC, Divide",  # op 7707
	"SPA CLA    Skip on AC >= 0, Clear AC",  # op 7710
	"UNKNOWN!   ILLEGAL!",  # op 7711
	"SPA CLA HLTSkip on AC >= 0, Clear AC, Halt",  # op 7712
	...
	"SPA LAS HLTSkip on AC >= 0, Load AC from the Switch Register, Halt",  # op 7716
	" ACL LSR   Load MQ into AC, Logical shift right",  # op 7717
	"SMA SNL CLASkip on AC < 0 or L = 1, Clear AC",  # op 7720
	"CLA SWP    Load AC from MQ then clear MQ",  # op 7721
	"SMA SNL CLA HLTSkip on AC < 0 or L = 1, Clear AC, Halt",  # op 7722
	"CLA SWP SCLLoad AC from MQ then clear MQ, Step counter load",  # op 7723
	"SMA SNL LASSkip on AC < 0 or L = 1, Load AC from the Switch Register",  # op 7724
	"CLA SWP MUYLoad AC from MQ then clear MQ, Multiply",  # op 7725
	"SMA SNL LAS HLTSkip on AC < 0 or L = 1, Load AC from the Switch Register, Halt",  # op 7726
	"CLA SWP DVILoad AC from MQ then clear MQ, Divide",  # op 7727
	"SPA SZL CLASkip on AC >= 0 and L = 0, Clear AC",  # op 7730
	"UNKNOWN!   ILLEGAL!",  # op 7731
	"SPA SZL CLA HLTSkip on AC >= 0 and L = 0, Clear AC, Halt",  # op 7732
	"CLA SWP SHLLoad AC from MQ then clear MQ, Shift left",  # op 7733
	"SPA SZL LASSkip on AC >= 0 and L = 0, Load AC from the Switch Register",  # op 7734
	"CLA SWP ASRLoad AC from MQ then clear MQ, Arithmetic shift right",  # op 7735
	"SPA SZL LAS HLTSkip on AC >= 0 and L = 0, Load AC from the Switch Register, Halt",  # op 7736
	"CLA SWP LSRLoad AC from MQ then clear MQ, Logical shift right",  # op 7737
	"SMA SZA CLASkip on AC <= 0, Clear AC",  # op 7740
	"UNKNOWN!   **Unknown**",  # op 7741
	"SMA SZA CLA HLTSkip on AC <= 0, Clear AC, Halt",  # op 7742
	"UNKNOWN!   **Unknown**",  # op 7743
	"SMA SZA LASSkip on AC <= 0, Load AC from the Switch Register",  # op 7744
	"UNKNOWN!   **Unknown**",  # op 7745
	"SMA SZA LAS HLTSkip on AC <= 0, Load AC from the Switch Register, Halt",  # op 7746
	"UNKNOWN!   **Unknown**",  # op 7747
	"SPA SNA CLASkip on AC > 0, Clear AC",  # op 7750
	"UNKNOWN!   ILLEGAL!",  # op 7751
	"SPA SNA CLA HLTSkip on AC > 0, Clear AC, Halt",  # op 7752
	"UNKNOWN!   **Unknown**",  # op 7753
	"SPA SNA LASSkip on AC > 0, Load AC from the Switch Register",  # op 7754
	"UNKNOWN!   **Unknown**",  # op 7755
	"SPA SNA LAS HLTSkip on AC > 0, Load AC from the Switch Register, Halt",  # op 7756
	"UNKNOWN!   **Unknown**",  # op 7757
	"SMA SZA SNL CLASkip on AC <= 0 or L = 1, Clear AC",  # op 7760
	"UNKNOWN!   **Unknown**",  # op 7761
	"SMA SZA SNL CLA HLTSkip on AC <= 0 or L = 1, Clear AC, Halt",  # op 7762
	"DLD		 Double precision loadMode B)",  # op 7763
	"SMA SZA SNL LASSkip on AC <= 0 or L = 1, Load AC from the Switch Register",  # op 7764
	"UNKNOWN!   **Unknown**",  # op 7765
	"SMA SZA SNL LAS HLTSkip on AC <= 0 or L = 1, Load AC from the Switch Register, Halt",  # op 7766
	"UNKNOWN!   **Unknown**",  # op 7767
	"SPA SNA SZL CLASkip on AC > 0 and L=0, Clear AC",  # op 7770
	"UNKNOWN!   ILLEGAL!",  # op 7771
	"SPA SNA SZL CLA HLTSkip on AC > 0 and L=0, Clear AC, Halt",  # op 7772
	"CLA DPIC   Clear AC, Double precision incrementMode B)",  # op 7773
	"SPA SNA SZL LASSkip on AC > 0 and L=0, Load AC from the Switch Register",  # op 7774
	"CLA DCM    Clear AC, Double precision complementMode B)",  # op 7775
	"SPA SNA SZL LAS HLTSkip on AC > 0 and L=0, Load AC from the Switch Register, Halt",  # op 7776
